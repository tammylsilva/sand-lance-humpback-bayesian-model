...
for (i in 1:3){
alpha1[i] ~ dgamma(0.01,0.01)} #try block specific variances as requested by reviewer2 to better enable site comparisons
#alpha1 ~ dgamma(0.01,0.01) #original model in ms - Gelman - uniforms on standard deviation, or inverse gamma on precision, gamma on variance
alpha2 ~ dgamma(0.01,0.02) #smaller variance help autocorrelation?

#site level effect sl
for (i in 1:43){
site_sl[i] ~ dnorm(0, alpha1[block[i]])} #site variance varies by block, trying for revisions
...
#variance for random effects
for (i in 1:3){
alpha1w[i] ~ dgamma(0.01,0.01)} #try block specific variances as requested by reviewer to better enable site comparisons
...
#site level effect w
for (i in 1:43){
site_w[i] ~ dnorm(0, alpha1w[block[i]])} #instead of 0.01, include parameter that determines precision, trying for revisions
                               #constrain site, site specific deviations
#site_w[i] ~ dnorm(0, alpha1w)
...
#save model output
save.image(file = "model_revision.RData")
...
load("./results/bayesian_model/model6_FINAL_Oct2.RData")  #original model 
load("./results/bayesian_model/revisions/model_revision.RData") #trying block specific variance as suggested for revision
...
#need to convert alpha value precisions to variances before summarizing for reporting

#convert samps2 to dataframe
samps2df = as.data.frame(rbind(samps2[[1]], samps2[[2]],samps2[[3]], samps2[[4]]))

#get reciprocals - for original model (prevision = 1 / variance)
samps2df$alpha1_var <- 1 / samps2df$alpha1
samps2df$alpha1w_var <- 1 / samps2df$alpha1w
samps2df$alpha2_var <- 1 / samps2df$alpha2
samps2df$alpha2w_var <- 1 / samps2df$alpha2w

#for revisions
#samps2df$alpha11_var <- 1 / samps2df$`alpha1[1]`
#samps2df$alpha12_var <- 1 / samps2df$`alpha1[2]`
#samps2df$alpha13_var <- 1 / samps2df$`alpha1[3]`
#samps2df$alpha1w1_var <- 1 / samps2df$`alpha1w[1]`
#samps2df$alpha1w2_var <- 1 / samps2df$`alpha1w[2]`
#samps2df$alpha1w3_var <- 1 / samps2df$`alpha1w[3]`
#samps2df$alpha2_var <- 1 / samps2df$alpha2
#samps2df$alpha2w_var <- 1 / samps2df$alpha2w

#turn dataframe back into a matrix - required for MCMCsummary
samps2mat <- as.matrix(samps2df)

#get summary for results table for paper - note, will not produce n.eff or rhat because individual chains aren't recognizable
#when in matrix form
prectovar <- as.data.frame(MCMCsummary(samps2mat, round=2,n.eff=TRUE,func=median,func_name="median"))

#save results table as csv
write_csv(prectovar,"precision_to_variances_for_alphas_rev.csv")

#save posterior distributions for alpha1 and alpha1w for block specific variances
#for model revisions
MCMCtrace(samps2, params='alpha1', type='density',sz_tick_txt=1.4,pdf=FALSE)
